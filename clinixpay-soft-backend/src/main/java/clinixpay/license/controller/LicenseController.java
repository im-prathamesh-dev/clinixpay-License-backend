...
public class LicenseController {
    ...
    public ResponseEntity<String> cancelPayment(@PathVariable String userId) {
        try {
            String result = licenseService.deletePendingUser(userId);

            if ("ALREADY_ACTIVE".equals(result)) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                        .body("Cannot delete this user because the payment was successful and the license is already ACTIVE.");
            }

            if ("DELETED".equals(result)) {
                return ResponseEntity.ok("Pending payment record cleaned up successfully.");
            }
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body("An error occurred during cleanup.");
        }
    }
